	...
	public static List<Course> getCourses(HttpSession sc) {
		...
		try {
			...
			while (courseRes.next()) {
				...
				// Sections
				try (PreparedStatement sectionStmt = con
						.prepareStatement("select * from course_sections where course_id=?")) {
					sectionStmt.setString(1, courseId);
					try (ResultSet sectionRes = sectionStmt.executeQuery()) {
						ArrayList<String> sections = new ArrayList<>();
						while (sectionRes.next()) {
							sections.add(sectionRes.getString("section_id"));
						}
						// Preferred teachers
						try (PreparedStatement prefStmt = con
								.prepareStatement("select * from course_preferred_teachers where course_id=?")) {
							prefStmt.setString(1, courseId);
							try (ResultSet preferdRes = prefStmt.executeQuery()) {
								ArrayList<Integer> prefs = new ArrayList<>();
								while (preferdRes.next()) {
									prefs.add(preferdRes.getInt("teacher_id"));
								}
								// Forbidden teachers
								try (PreparedStatement forbStmt = con.prepareStatement(
									...
									try (ResultSet forbiddenRes = forbStmt.executeQuery()) {
										ArrayList<Integer> forbidds = new ArrayList<>();
										while (forbiddenRes.next()) {
											forbidds.add(forbiddenRes.getInt("teacher_id"));
										}

										Course c = new Course(courseId, CourseType.valueOf(courseType),
												new HashSet<>(prefs), new HashSet<>(sections), new HashSet<>(forbidds),
